...
module.exports = (app) => {
  ...
  app.use((error, req, res, next) => {
    ...
    else if (error instanceof mongoose.Error.CastError) error = createError(404, 'Recurso no encontrado')
    else if (error instanceof jwt.JsonWebTokenError) error = createError(401, error)
    else if (error.message.includes('E11000')) error = createError(400, 'El recurso ya existe')
